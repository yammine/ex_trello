defmodule ExTrello.API.Lists do
  ...
  defapicall list(id, options) when is_binary(id) do
  ...
  defapicall create_list(board_id, name, options) when is_binary(name) do
    request(:post, "lists", [{:name, name}, {:idBoard, board_id}|options])
    |> Parser.parse_list
  end

  def edit_list(%List{id: id}, fields), do: edit_list(id, fields)
  defapicall edit_list(id, fields) when is_binary(id) and is_list(fields) do
